...
funkcije $\Loss{homo}$. Za vsak element $g$ iz grupe $G$ izberemo svojo
matriko $f(g)$ in s tem definiramo preslikavo
$f \colon G \to \C^{n \times n}$. Če je za tako preslikavo $f$
vrednost funkcije
izgube $\Loss{homo}(f)$ enaka nič, je $f$ upodobitev.
\begin{primer}
  {
    Poljubno preslikavo iz grupe $\Z_2 = \{0, 1\}$ v $\C$
...
\emph{izomorfizmov}
grafov. Homomorfizme grup v permutacije oziroma izomorfizme grafov
enačimo
z množico ničel gladke funkcije
$\loss \colon \R^n \to [0, \infty)$. S tem iskanje diskretnih struktur
...
V tem primeru lahko gradientni spust obravnavamo kot numerično metodo
za reševanje
diferencialne enačbe
$\frac{d\phi}{dt} = - \nabla \loss(\phi)$ in študiramo dinamiko
rešitev te enačbe.
V delu se poglobimo v enačbe, ki pripadajo iskanju nerazcepnih
...
gladko družino porazdelitev in za slučajno permutacijo $f \in S_n$
maksimiziramo verjetnost, da je $f$ homomorfizem. S tem
pristopom se ognemo pretvorbi dvojno stohastične matrike v permutacijsko,
...
metodo razširimo z uporabo tabele inverzij, prek katere
...
Metode v delu so izvirni rezultati
avtorja, mentorja in somentorja. Podoben pristop, kot
ga uporabimo za iskanje nerazcepnih upodobitev končnih grup, avtorji v članku
~\cite{chughtai2023neural} uporabljajo
...
\begin{tabularx}{\linewidth}[ht]{l X}
...
minimuma $$\hat \phi = \operatorname*{argmin}_{\phi} \mathcal L(\phi)$$
funkcionala $\loss$. Če je domena funkcionala evklidski
...
\begin{primer}
  ...
  \begin{figure}[ht]
    ...
    \caption[Primer delovanja gradientnega spusta.]{Sto korakov
      ...
      funkcije.
      Študiramo lahko, od kje naprej se funkcijske vrednosti
      od lokalnega minimuma razlikujejo
      za manj kot $\epsilon = 0.1$. Pri
      $\eta = 0.1$ se to zgodi po 12 korakih, pri $\eta = 0.5$ pa
      po treh.
    }
...
\begin{opomba}
  ...
  dobimo metodo, ki z visoko verjetnostjo pobegne iz
  sedel~\cite{jin2017escapesaddlepointsefficiently}. Te lastnosti
  ...
  (glej~\cite{dherin2025learningsolvingdifferentialequations}
...
\begin{align*}
  ...
  v_{i+1} &= \gamma \cdot v_i + (1-\gamma) (\nabla \loss (\phi_i))^2 \\
  \overline{v}_{i+1} &= \frac{v_{i+1}}{1 - \gamma ^{i+1}}\\
  \phi_{i+1} &= \phi_i -  \eta
  \frac{\overline{m}_{i+1}}{\sqrt{\overline{v}_{i+1}} + \epsilon},
\end{align*}
kjer so $\eta > 0, \beta \in [0,1), \gamma \in
...
Iz sedla lahko pridemo z dodajanjem slučajne perturbacije
parametrov, ko je gradient ciljne funkcije
majhen~\cite{jin2017escapesaddlepointsefficiently}.
...
enakomerno porazdeljen šum $|\xi_i| \leq r $
...
Množico preslikav končne grupe $G$ v prostor matrik $\R^{n \times n}$ lahko
enačimo\footnote{Za $G=\{g_1, g_2, \dotsc, g_m\}$ lahko
  enačimo $f = (\operatorname{flatten}(f(g_1)),
    \operatorname{flatten}(f(g_2)), \dotsc,
\operatorname{flatten}(f(g_{m}) ))$.} z evklidskim prostorom $K = \R^{n^2 |G|}$.
...
\ref{subsection:ciklicne-grupe} in \ref{subsection:diedrske-grupe}
študiramo dinamiko diferencialnih enačb gradientnega toka
...
%
\begin{definicija}
  \label{def:razsiritev-repr-modela}
  Preslikavo $\hat \rho$ definiramo na grupi $G$ na naslednji način:
  \begin{itemize}
...
\begin{definicija}
  ...
  kjer so $v_i, u_i \in S^*$, $r_{l_i} \in R$ in
  $v_i \equiv u_i^{-1}  \mod G$, $L(w) \in \N$.

  Če zgornja oblika zapisa ni enolična, izberemo tako, da se
  pred v $v_{i-1} u_i r_i$ ne pojavi nobena druga beseda iz $R$ razen $r_i$.

  Z $L(w)$ označimo \emph{število pojavitev}
  besed iz $R$ v
  $w$ in definiramo
  $$L=\max_{g \in G} L(w_{g^{-1}} w_g),$$
  ki vpliva na
...
\begin{primer}
  ...
  $.
  Predstavljamo si lahko, da $a$ predstavlja transpozicijo $(12)$,
  $b$ pa transpozicijo $(13)$.
  Model je definiran
  z vrednostima
...
\begin{opomba}
  ...
  transpozicija in $n$-cikel), operiramo le s štirimi
...
\begin{opomba}
  ...
  zaprti obliki~\cite{park2024absenceclosedformdescriptionsgradient}.
  Običajno se jih
  računa numerično.
  ...
  tokove (ciklične in diedrske grupe)
  integriramo
  z metodo Dormand-Prince
  \cite{solvingordinarydifferentialequations1993nonstiff}. Uporabljamo
  implementacijo iz knjižnice \texttt{SciPy}~\cite{2020SciPy-NMeth}.
  Enačbe zapletenih
  gradientnih tokov rešujemo z algoritmom Adam, kjer uporabljamo
  implementacijo iz knjižnice \texttt{PyTorch}~\cite{paszke2019pytorch}.
\end{opomba}
%%%%%%%%%%%%
...
Zanima nas, ali velja obrat. Lahko iz majhne napake modela
$\hat \rho$ sklepamo na obstoj
...
\begin{izrek}[Obstoj rešitve]
  ...
  Obstaja $\delta(\epsilon) = O(\epsilon)$, da iz
  $
  \loss(\hat \rho) < \delta(\epsilon)
  $
  sledi, da
  obstaja nerazcepna upodobitev $\rho \colon G \to
  \operatorname{GL}_n(\C) $, za katero velja
  $$
...
\begin{lema}
  ...
  Iz  $\Loss{unitary}(\hat \rho) < |S|\epsilon $ sledi, da je
...
\begin{dokaz}
  ...
  \dotsc, \sigma_n)  - I\|
  = \sqrt{\sum_{i=1}^n (\sigma_i -1)^2}.
  $$
  Podobno je
  $$
  \|\s \s^* - I\| = \sqrt{\sum_{i=1}^n (\sigma_i^2 -1)^2}
  $$
  in očitno velja
  $
...
\end{dokaz}
Če je $\Loss{unitary}(\hat\rho) < |S|\epsilon$, lahko zapišemo
...
\begin{lema}
  ...
  $\Loss{rel}(\hat \rho) < |R| \epsilon$ in $
  \Loss{unitary}(\hat \rho)  < |S| \epsilon$.
  Potem za taki besedi $w, v \in S^*$, za kateri velja
...
\end{lema}
\begin{dokaz}
  Opazimo, da za $A, B \in \operatorname{GL}_n$ iz
  $\|AB-I\| < \delta$ sledi
...
\begin{dokaz}
  ...
  velja $w_g w_h w_{gh}^{-1} \equiv 1 \mod R$.
  Zato je po lemi \ref{lema : zveznost na besedah}
  $$
  \|\hat \rho(w_g w_h)\hat \rho(w_{gh})^{-1} - I\| = O(\epsilon).
  $$
  Ker je za dovolj majhen $\epsilon$ matrika $\hat \rho(w_{gh})$ obrnljiva,
  iz tega sledi
  $$
  \|\hat \rho(w_g w_h) - \hat \rho(w_{gh})\| = O(\epsilon),
  $$
  kar je enakovredno trditvi
  $\|\hat \rho(gh) - \hat \rho(g)\hat \rho(h)\| = O(\epsilon)$.
\end{dokaz}
%
% \begin{lema}
...
\begin{lema}
  Za preslikavo $\mu \colon G \to U(n)$, inducirano iz
...
sledi $\|\mu(g) - \hat \rho(g)\| = O(\epsilon)$.
...
\begin{opomba}
  ...
  kjer je $U$ odprta v $\R^n$. Če najdemo $W = \{a\}$, je dokaz končan.

  Če take $W$ ni, potem  za poljubno smer $w$ in  za dovolj majhne $t$ velja
...
\label{subsection:ciklicne-grupe}
...
$$
kjer je $k \in \{0, 1, \dotsc, n-1\}$. Pri iskanju teh upodobitev
...
\begin{figure}[ht]
  ...
  {Rešitve enačb gradientnega
    ...
    parametre\protect\footnotemark.
...
\end{figure}
\footnotetext{Na vsakem grafu je 200 različnih rešitev. Začetni parametri so
  vzorčeni iz enakomerne porazdelitve nad $[-1,1]^2$. Za implementacijo
glej dodatek \ref{dodatek_1dim dn trajektorije}.}
%
%
%
...
\end{figure}\footnotetext{Začetni parametri $R_0, S_0 $ so oblike $
  \begin{bmatrix}
    x & y \\
    z & \operatorname{tr} - x
\end{bmatrix}$, kjer so $x,y,z, \operatorname{tr} \sim U[-2,2]$}
...
%
% \end{align*}
Očitno velja $\frac{d}{ds}\Loss{rel}(0,z) =
\frac{d}{d_z}\Loss{rel}(s,0) = 0  $.
Rešitev z začetnima parametroma $s_0 = 0, z_0 \in \R$ je oblike
$$
s(t) = 0,
$$
$z(t)$ pa rešuje pripadajočo enodimenzionalno enačbo gradientnega toka.
Simetrično velja za $z_0 = 0$, kar ustreza rezultatu za diedrske grupe.

Preučimo še dogajanje znotraj kvadrantov. Recimo, da je
...
\begin{bmatrix}
  \frac{1}{s} & 0 \\
  0 & \frac{1}{z}
\end{bmatrix}$. Kritične točke funkcij $\Loss{rel}$ in $F$ se ujemajo.
...
verjetnost za homomorfizem čim večja. V strojnem
...
(\text{opaženi izidi}))$~\cite{prince2023understandingdeeplearning}.
...
\begin{definicija}
  ...
  \ref{def:razsiritev-repr-modela} razširimo na
  celotno grupo:
  \begin{itemize}
...
$g = s_1 s_2 \dotsm s_m$.
Lahko preverimo, da za $g = s_1 s_2 \dotsm s_m \in G$ velja $$
...
\end{dokaz}
\begin{lema}[Za dovolj majhen $\epsilon$ je $P_s$ obrnljiva]
...
\begin{dokaz}
  ...
  Induktivno nadaljujemo.

  Izberemo tak $\epsilon$
...
$P_s = \operatorname{softmax}(S)$, $R, S \in \R^{m \times m}$
in $\log$ logaritem matrike po elementih.
\paragraph{Rezultati}
...
$G_1$ in $G_2$, za katero velja
$$f(i) \sim_2 f(j) \iff i \sim_1 j.$$
...
\end{align*}
kjer $\operatorname{Perm}(P_f)$ označuje
\emph{permanent}\footnote{$\operatorname{Perm}(A) = \sum_{\sigma  \in
...
\begin{definicija}
  ...
  na $n$ vozliščih. Problem ujemanja grafov
  \emph{(graph matching)}~\cite{lyzinski2015graphmatchingrelaxrisk} je
  definiran kot
  iskanje minimuma
  $$
...
izomorfizma grafov pretvorimo v dejanski izomorfizem, če je le
...
\end{dokaz}
\begin{lema}
  \label{lema:ocena-za-graph-matching}
  Za preslikavo
...
\begin{lema}
  Če je
  $$\Loss{unitary}(P) \le \epsilon \quad \text{in} \quad
...
\begin{dokaz}
  ...
  Po lemi \ref{lema:ocena-za-graph-matching}
  pa velja
  $$
  ...
  vozlišča grafa $M_2$ preslikamo s permutacijo $X$.
...
\begin{primer}
  Na sliki \ref{fig:primer_grafa} je preprost graf na
  ...
  \begin{figure}[ht]
    ...
    \caption{Preprost graf na petih vozliščih.}
    \label{fig:primer_grafa}
  \end{figure}
  Gradientni spust poleg trivialne rešitve na določenih
  začetnih vrednostih  parametrov uspe najti avtomorfizem $(03)(12)$,
  na drugih pa konvergira do limite, ki ni blizu avtomorfizma. Slika
  \ref{fig:primer_grafa_parametri} prikazuje konvergenco na različnih
  začetnih vrednostih parametrov matrike $P$.
  \begin{figure}
    \begin{subfigure}{\textwidth}
      \centering
      \includegraphics[height=0.3\linewidth]{images/random_samples_big_random_graph_1_on_5sample_9_initial.pdf}
      %\hfill
      \includegraphics[height=0.3\linewidth]{images/random_samples_big_random_graph_1_on_5sample_9_final.pdf}
      \caption{Konvergenca do identitete.
      }
    \end{subfigure}
    \begin{subfigure}{\textwidth}
      \centering
      \includegraphics[height=0.3\linewidth]{images/random_samples_big_random_graph_1_on_5sample_0_initial.pdf}
      %\hfill
      \includegraphics[height=0.3\linewidth]{images/random_samples_big_random_graph_1_on_5sample_0_final.pdf}
      \caption{Konvergenca do avtomorfizma $(03)(12)$.
      }
    \end{subfigure}
    \begin{subfigure}{\textwidth}
      \centering
      \includegraphics[height=0.3\linewidth]{images/random_samples_big_random_graph_1_on_5sample_11_initial.pdf}
      %\hfill
      \includegraphics[height=0.3\linewidth]{images/random_samples_big_random_graph_1_on_5sample_11_final.pdf}
      \caption{Neuspešna konvergenca. 
      }
    \end{subfigure}
    \caption{Vrednosti parametrov matrike $P$ na začetku in na koncu
      optimizacije. Različne vrednosti začetnih parametrov
    vplivajo na uspešnost konvergence. }
    \label{fig:primer_grafa_parametri}
  \end{figure}
  %
\end{primer}
...
Oštevilčenje vozlišč (elementov  $S_n$)
sledi metodi \\
...
Radi bi, da je za vsak par vozlišč $i,j$ verjetnost
...
Za par vozlišč $i,j$ in permutacijo $f\in S_n$ namreč lahko
...
pri metodi s tabelo inverzij pa funkcijo izgube
...
\begin{figure}[hp]
  ...
  \caption{Primerjava izgube $\loss_\sim(\overline{P})$
    ...
    Metodi sta preizkušeni na istih $10$-tih
    naključno izbranih grafih s fiksnim številom vozlišč. Vsaka točka
    je povprečje
...
sedla bolj razširjena od lokalnih minimumov. Z večanjem
števila parametrov
se v reliefu funkcije izgube pojavlja več sedel, iz katerih
lahko pobegnemo s perturbiranim gradientnim spustom,
...
Slika \ref{fig:loss_computation_time_comparison}
prikazuje primerjavo metode brez tabele inverzij, predstavljene
v poglavju \ref{section:prehod na izomorfizen vanilla},
in metode s tabelo inverzij brez preparametrizacije,
predstavljene v poglavju \ref{funkcija izgube TI}.
Metodi sta preizkušeni na istih grafih in prikazana je rast
povprečne izgube glede na število vozlišč.
...
Vse metode so preizkušene na
$10$ naključno izbranih grafih s fiksnim številom vozlišč.
Slika \ref{fig:primerjava vanilla-nn-mild-nn}  prikazuje rast
povprečne izgube metod glede
na število vozlišč v grafu.
...
\begin{table}
  ...
  \begin{tabular}{l|S[table-format=2.2]}
    ...
    Globoka preparametrizacija & {\bfseries 10.94} \\
...
zastavi splošno vprašanje, ali morajo biti rešitve
podobnih enačb podobne?

Za $\epsilon$-upodobitve amenabilnih grup je odgovor pritrdilen.
Kazhdanov izrek \ref{izrek : kazhdan} pravi,
da je za dovolj majhen $\epsilon$ vsaka
$\epsilon$-upodobitev amenabilne grupe blizu pravi upodobitvi.

V delu se ukvarjamo s sorodnim problemom \emph{stabilnosti prezentacij},
omenjenim v
...
Vprašanje je, ali se da dokaz razširiti na nekatere neskončne
grupe in kakšne pogoje morajo izpolnjevati.
Ta smer ostaja odprta, saj v literaturi nismo našli jasnih rezultatov
za ta primer.

Dokaz Kazhdanovega izreka za splošne amenabilne grupe je daleč od trivialnega.
...
Alternativa, ki je popularna v literaturi pri ujemanju grafov je
...
perturbiran gradientni spust, s čimer se izognemo sedlom
...
preferiramo pred
drugimi. To bi lahko dosegli z dodajanjem asimetričnega člena
...
funkcijami (permutacijami) tako, da bi se ognili lokalnim minimumom.
...
možnih pristopov k reševanju ujemanja grafov bi bil, da poiščemo
...
se v obeh primerih pojavljajo lokalni minimumi.
\appendix
\clearpage
...
\begin{definicija}[Nevronska mreža]
  ...
  b_1, \dotsc, \Omega_l, b_l$. Če je $l \geq 2$, rečemo, da
  je mreža globoka.
\end{definicija}
...
z gradientnim spustom vedno najdejo parametre, ki dosegajo
...
\begin{izrek}[Allen-Zhu, Li, Song]
  ...
  Naj $f(z,y)$ izpolnjuje Polyak-Lojasiewiczev pogoj.
  Naj bo $S_\text{train}$ taka množica vzorcev, da
  obstaja $\delta > 0$, da za dva vzorca
  $$(x_i, y_i),
  (x_j,y_j) \in S_\text{train}$$
  z različnima indeksoma $i
...
\begin{definicija}
  ...
  \label{def:sinkhorn}
  Naj bo $K \in \R^{n \times n}$ matrika s pozitivnimi
...
in aktivacijsko funkcijo $\operatorname{ReLU}$ z naslednjim predpisom:
$$
...
\begin{definicija}
  ...
  grafovske nevronske mreže na naslednji
  način uporabimo za
...
\subsection{Upodobitve diedrskih grup}
...
\begin{figure}[!hp]
  \centering
  \includegraphics[width=0.77\textwidth]{./images/dihedral/grid/cut_plot_grid_n3_-1.5_-0.5_r2000.pdf.png}
  \medskip
  \includegraphics[width=0.74\textwidth]{./images/dihedral/grid/cut_plot_grid_n3_0.5_1.5_r3000.pdf.png}
  \caption{Hitrosti konvergence enodimenzionalnega modela
    $D_3 \to \R$ na
...
%
\clearpage
\subsubsection{Dvodimenzionalen model}
...
\end{figure}
\clearpage
\subsection{Delovanja ciklične grupe}
...
\end {figure}
\clearpage
\subsection{Delovanja diedrske grupe}
