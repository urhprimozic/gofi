...
%%%%%%%%%%%%
\begin{opomba}
  \TODO{Po mojem to drži.}
  Če nas ne zanima eksplicitna meja $\epsilon$, da iz
  $\Loss(\hat \rho) < \epsilon$, ampak samo njen obstoj,
  je dokaz veliko lažji.
  %Opazujmo preslikavo
  %$\Loss{2} (\hat \rho) = \sum_{r \in R} \left\|
  %\hat \rho(r) - I
  %\right\|^2$. (TO JE POLINOM)
  Ker je prostor unitarnih matrik kompakten, izven neke kompaktne okolice 
  $\Loss{unitary}$ ni poljubno majhna. Če nas zanima, ali so lahko 
  vrednosti izgube poljubno majhne (a neničelne), se lahko omejimo na 
  nek kompakt.

  $\Loss{rel}$ je zvezna funkcija na tem kompaktu. 
  ...
  neničelni minimum $c > 0$. Če je $\Loss{rel} \hat \rho < c$,
...
$(\operatorname{tr}(R(t)), \operatorname{tr}(S(t)))$.
    ...
    \hat\rho(s)$ definira slučajno preslikavo\footnote{Zloraba
      ...
      \begin{opomba}
        ...
        elementi matrike $\phi_s$ vzorčeni enakomerno na intervalu $[0,1]$. Pričakovana vrednost
    ...
    $$
   % \begin{opomba}
      Očitno je $$1-P(i \sim j \iff f(i) \sim f(j)) = (M_1)_{i,j} - P(f(i) \sim f(j)).$$
