...
$(\operatorname{tr}(R(t), \operatorname{tr(S(t)})$.
    ...
    Uvedemo $F_i(x,y) = (e^{a_ix + b_iy} -1)^2$. Za poljuben vektor
    $v \in \R^2$ uvedemo
    ...
    Za $t>0$ velja $\nabla F(tv) = \sum_{i=1}^l f_i'(t) = 0$ le, če
    je $f_i'(t) = 0$ za vsak $i$. To se zgodi le, če je $t=0$ ali
    $\lambda_i = 0$ za vsak $i$. Druga možnost pa ni mogoča, saj bi
    pomenila, da so vsi vektorji $(a_i, b_i)$ pravokotni na isti
    vektor $v$, kar se ne zgodi, saj $R$ vsebuje člena 
    oblike $s^{a_i}$ in $z^{b_j}$.

    Edina stacionarna točka $F$ je torej $x=y=0$, kar ustreza $s=z=1$.
