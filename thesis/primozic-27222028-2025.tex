...
kanonične predstavnike $w_g$ izb%GOTO
\begin{lema}
...
\begin{opomba}
  ...
  $\Loss{rel}(\hat \rho) < \epsilon$, ampak samo njen obstoj,
  je dokaz veliko lažji.
  %Opazujmo preslikavo
  %$\Loss{2} (\hat \rho) = \sum_{r \in R} \left\|
  %\hat \rho(r) - I
  %\right\|^2$. (TO JE POLINOM)
  $\Loss{rel}$ je zvezna funkcija na končnem prostoru
  $GL_n(\C)^S \equiv \R^{2|S|n^2}$. Preprosto je dokazati, da je
...
$(\operatorname{tr}(R(t)), \operatorname{tr}(S(t)))$.
    ...
    [
      ...
      \dotsc ,
      f(n)
    ]^T \in \R^n$. Lahko bi začeli s poljubno
