...
V tem primeru lahko gradientni spust obravnavamo kot numerično metodo za reševanje
...
parametrov, ko je gradient ciljne funkcije majhen~\cite{jin2017escapesaddlepointsefficiently}.
...
\begin{izrek}[Obstoj rešitve]
  ...
  Obstaja $\delta(\epsilon) \leq O(\epsilon)$, da iz
  $
...
\end{align*}
kjer $\operatorname{Perm}(P_f)$ označuje \emph{permanent}\footnote{$\operatorname{Perm}(A) = \sum_{\sigma  \in
...
\begin{figure}[hp]
  ...
  \caption{Primerjava izgube $\loss_\sim(\overline{P})$
    ...
    naključno izbranih grafih s fiksnim številom vozlišč. Vsaka točka je povprečje
...
Slika 
\ref{fig:loss_computation_time_comparison}
prikazuje primerjavo uporabe metode brez tabele inverzij, predstavljene
v poglavju \ref{section:prehod na izomorfizen vanilla},
z uporabo tabele inverzij
brez preparametrizacije, predstavljene v poglavju 
\ref{funkcija izgube TI}
. Metodi sta preizkušeni na istih grafih. Slika \ref{fig:loss_computation_time_comparison} prikazuje 
rast povprečne izgube metod glede na število vozlišč v grafu.
...
Vse metode so preizkušene na 
 $10$-tih
naključno izbranih grafih s fiksnim številom vozlišč. 
Slika \ref{fig:primerjava vanilla-nn-mild-nn}  prikazuje rast povprečne izgube metod glede 
