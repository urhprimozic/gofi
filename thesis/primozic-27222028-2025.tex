...
Obstaja očitna\footnote{Za $G=\{g_1, g_2, \dotsc, g_m\}$ lahko
  enačimo $f = (\operatorname{flatten}(f(g_1)),
    ...
    \begin{izrek}
      ...
      \end{definicija}
      \begin{opomba}
        Ker so vse grupe v tem delu končne\footnote{V splošnem je $w_g \in \mon$.
      ...
      \begin{izrek}[Obstoj rešitve]
        ...
        $
        sledi, da
        obstaja upodobitev $\rho \colon G \to
      ...
      $\Loss{unitary}(\hat \rho)$ lahko sklepamo na 
      omejeno normo $||\hat \rho(s)||$ za vsak $s \in S$.ž
\begin{lema}
        Za $\epsilon>0$ obstaja $\delta>0$, 
       da iz  $\Loss{unit}(\hat \rho) < \delta $ sledi, da je 
       $||\hat \rho(s) - U_s|| < \epsilon$ za $s \in S$ in 
       $U \in U_n(\C)$.    
      \end{lema}
      \begin{dokaz}
        Opazujmo preslikavo 
      ...
      Najprej pokažemo, da je model z majhno napako
      skoraj homomorfizem.
      ...
      \begin{dokaz}
        ...
        velja $w_g w_h  \equiv w_{gh} \mod G$.
        \TODO{sledi iz zgornjega...}
      \end{dokaz}
      ...
      \TODO{
        ...
        \begin{enumerate}
          \item done
          \item je tud $\Loss{homo}(\hat \rho)$ majhen , iz zveznosti
            gre pol to tud na $\mu$.
        \end{enumerate}

      }
      %
     % \subsubsection{Razširitev modela na celotno grupo}
      %\label{subsection : razširitev repr modela}
     %
      %Definicija je dobra le, če so vse matrike $P(s)$ obrnljive. Po
      %spodnji lemi
      ...
      \end{dokaz}
      % Tole je narobe:
      % r = 1, ion ga p o def slikam v 1.
      % lahko pa definiram zadevo na F_S in tm to računam...
      % pravino je tkole:
      % najprej dokažem ,da se bi ekvivalentne besede slikajo blizu
      % pol gledam, kam se slika beseda ki predstavlja r in ni 1, pa se moree
      % slikat blizu 1
      ...
      % se vsak generator $s$ pojavi v neki relaciji $r \in R$, torej
      % so za dovolj majhno funkcijo izgube
      % vse matrike $\hat \rho(s)$ obrnljive. S tem je definicija
      % \ref{def:razširitev upodobitev} dobra.
%
      %\subsubsection{Dokaz izreka o obstoju rešitve}
 %     
%
          ...
          \hat\rho(s)$ definira slučajno preslikavo\footnote{Izraba
            ...
            $$P(s = f) = \prod _{i=1}^n P(s(i) = f(i)$$
              ...
              \begin{definicija}
                ...
                \ref{subsection : razširitev repr modela} raširimo na
                celotno grupo:
                \begin{itemize}
