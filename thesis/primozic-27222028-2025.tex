...
optimizacijsko metodo v strojnem učenju, ki je
...
$(\operatorname{tr}(R(t), \operatorname{tr(S(t)})$.
    ...
    koordinate $x_i = \log s_i$ in enako kot zgoraj dokažemo, da so edine kritične točke funkcije
    $$
    F(x_1, x_2, \dotsc, x_k) 
    = \sum_{i=1}^l (e^{a_1^{(i)}x_1 + a_2^{(i)}x_2 + \dotsc + a_k^{(i)}x_k} -1)^2.
    $$
     v 
    ...
    \begin{itemize}
      ...
      \item umestitev v evklidski prostor glede na napako (transformer, embedding)
