...
Obstaja očitna\footnote{Za $G=\{g_1, g_2, \dotsc, g_m\}$ lahko
  enačimo $f = (\operatorname{flatten}(f(g_1)),
    ...
    \begin{izrek}
      ...
      je opisana v poglavju \ref{subsection : razširitev na upodobitev}.
      ...
      \begin{izrek}[Obstoj rešitve]
        Naj bo $\hat \rho \colon G \to \C^{n \times n}$
        taka preslikava, da  za nek za $\epsilon < \frac{1}{100}$
        velja
        $
        \Loss{rel}(\hat \rho) < \epsilon
        $
        .
        Potem obstaja upodobitev $\rho \colon G \to
        GL_n(\C) $, da velja
        $$
        \sup_{s \in S} ||\hat \rho(s) - \rho(s) || \leq 2\epsilon.
        $$
      \end{izrek}
      ...
      \begin{dokaz}[Kazhdan]
        ...
        $$
      \end{dokaz}
      \subsubsection{Razširitev modela na upodobitev}
      \label{subsection : razširitev na upodobitev}
      \begin{definicija}
       % 
      %
      \label{def:razširitev upodobitev}
      Preslikavo $\hat \rho$ razširimo na celo grupo $G=<S|R>$ na slednji način:
      \begin{itemize}
        \item Za vsak $g \in G\setminus \{1  \}$ izberemo
          takega \emph{kanoničnega predstavnika} $w_g \in F_S$ izmed
          proste grupe nad $S$,
          ki predstavlja $g$ v $G$ in ima izmed vseh besed, ki
          predstavljajo $g$, najkrajšo
          dolžino\footnote{Očitno je $w_1$ prazen niz in $|g| = |g^{-1}|$.}.
          Dodatno zahtevamo $w_ {g^{-1}} = w_g^{-1}$.
        \item Za $g$ s kanoničnim predstavnikom 
        $w_g = s_1^{e_1} s_2^{e_2} \dotsm s_m^{e_m}$, kjer so
        $e_i \in \{ -1, 1\}$, 
        definiramo
        $$
        \hat \rho(g) = \hat \rho(s_1)^{e_1} \hat \rho(s_2)^{e_2} \dotsm
        \hat \rho(s_m)^{e_m}.
        $$
      \end{itemize}
      %
      \end{definicija}
      Definicija je dobra le, če so vse matrike $P(s)$ obrnljive. Po spodnji lemi
      to drži, če je napaka modela $\Loss{rel}(\hat \rho)$ dovolj majhna.
      \begin{lema}
        \label{lema:obrnljivost R}
        %GOTO
        Ostaja $\delta_0>0$, da za vsak $0 < \delta \leq \delta_0 $
        iz $\Loss{rel}(\hat \rho) < \delta$ sledi, da je 
        $\hat \rho(r)$ obrnljiva za vsak $r \in R$.
      \end{lema}
      \begin{dokaz}
        Opazujmo zvezno preslikavo $X \mapsto \det(X)$. Zaradi zveznosti 
        obstaja $\delta_0$, da je krogla $K_{\delta_0}(I) \subset \det ^{-1}(\frac{1}{2}, 1 + \frac{1}{2})$.
        Če je $\Loss{rel}(\hat \rho) < \delta_0$, potem za vsak $r \in R$ velja
        $\det(\hat \rho(r)) \in (\frac{1}{2} , 1 + \frac{1}{2})$, torej je 
        $\hat \rho(r)$ obrnljiva.
      \end{dokaz}
      Po definiciji \ref{def:razširitev upodobitev} 
      je za $r = s_1^{e_1} s_2^{e_2} \dotsm s_m^{e_m}$
      slika  
      $\hat \rho(r) = \hat \rho(s_1)^{e_1}
       \hat \rho(s_2)^{e_2} \dotsm \hat \rho(s_m)^{e_m}$. 
       Če je $\hat \rho(r)$ obrlnjiva, so očitno obrnljive
       tudi vse matrike $\hat \rho(s_i)$. Ker operiramo v končnih grupah 
       se vsak generator $s$ pojavi v neki relaciji $r \in R$, torej so za dovolj majhno funkcijo izgube 
       vse matrike $\hat \rho(s)$ obrnljive. S tem je definicija
        \ref{def:razširitev upodobitev} dobra.
      \TODO{
      Treba je dokazat še 
       \begin{enumerate}
        \item obstaja nek $ \mu $ blizu $\hat \rho$, ki je unitaren
        \item je tud $\Loss{homo}(\hat \rho)$ majhen , iz zveznosti gre pol to tud na $\mu$.
       \end{enumerate}
      ...
      $(\operatorname{tr}(R(t), \operatorname{tr(S(t)})$.
          ...
          \hat\rho(s)$ definira slučajno preslikavo\footnote{Izraba
            ...
            $$P(s = f) = \prod _{i=1}^n P(s(i) = f(i)$$
              ...
              %
              \begin{definicija}
                Preslikavo $\hat \rho$ lahko podobno kot v poglavju
                \ref{subsection : razširitev na upodobitev} raširimo na
                celotno grupo:
                \begin{itemize}
              ...
              \begin{dokaz}
                ...
                $\prod_{i=1}^m \overline{P_{s_i}} \in \Pi = 1$.
              \end{dokaz}
              %
