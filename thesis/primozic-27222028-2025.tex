...
Nerazcepne upodobitve, delovanja in izomorfizme grafov lahko
razumemo kot množico ničel gladke funkcije
$\loss \colon \R^n \to [0, \infty)$
nad
evklidskim prostorom. S tem iskanje diskretnih struktur
prevedemo na optimizacijo gladkih funkcij.

Optimizacije se lahko lotimo z gradientnim spustom, temeljno
...
lahko dinamiko rešitev in odvisnost konvergence od izbire
začetnih parametrov.

V delu predstavimo gradientne
optimizacijske metode in potrebno teoretično ozadje iz teorije
upodobitev končnih grup. Predstavimo
metodo iskanja nerazcepnih upodobitev z gradientnim spustom
in empirične rezultate na cikličnih in diedrskih grupah.
Definiramo metodi za iskanje delovanj in
izomorfizmov grafov z gradientnim spustom in opišemo
empirične rezultate. Iskanje izomorfizmov grafov
povežemo s problemov ujemanja grafov. Na koncu
metodo razširimo z uporabo tabele inverzij, preko katere
kandidate za izomorfizem iščemo med distribucijami nad $S_n$
...
ga uporabimo za iskanje nerazcepnih upodobitev končnih grup,
v \TODO{} uporabijo za iskanje upodobitev Lievih grup.

Ujemanja grafov se preko strojnega učenja lotijo v
\TODO{}.

Študiramo lahko
teoretične aspektne take optimizacije

\TODO{Kaj je zgodovinski uvod?}
  ...
  enačimo $f = (\operatorname{flatten}(f(g_1)),
    ...
    \begin{izrek}
      ...
      \begin{opomba}
        ...
        \hat\rho(gh)-\hat\rho(g) \hat\rho(h)||_F.
        $$
      ...
      \begin{dokaz}
        ...
        Računamo
        $$
        ||H_s - I||_F = ||\operatorname{diag}(\sigma_1, \sigma_2,
        \dotsc, \sigma_n)  - I||_F
        = \sum_{i=1}^n (\sigma_i -1)^2.
        $$
        Podobno je
        $$
        ||\s \s^* - I||_F = \sum_{i=1}^n (\sigma_i^2 -1)^2
        $$
        in očitno velja
        $
        ...
        Iz tega sledi
        $$
        ||\s - Q_s||_F = ||H_s - I||_F \leq ||\s \s^* - I||_F < \epsilon.
        $$

      \end{dokaz}
      Če je  $\Loss{unitary}(\hat\rho) < \epsilon$, lahko zapišemo
      $\s = Q_s + F_s$ za $||F_s||_F < \epsilon$. Iz tega sledi ocena
      %
      $$
      ||\s||_F \leq ||Q_s||_F + ||F_s||_F < \sqrt{n} + \epsilon.
      $$
      ...
      \begin{lema}
        ...
        $$
        ||\hat \rho(w) - I||_F <\left( 1 + (\sqrt{n} +
        \epsilon)^{|w|} |w| \epsilon \right)^L - 1
        $$
        % $$
        % ||\hat \rho(w) - I||_F < (\sqrt{n} + \epsilon)^{|w|} |w| \epsilon.
        % $$
      \end{lema}
      \begin{dokaz}
        ...
        $||\hat \rho(v r u) - I||_F < (\sqrt{n} + \epsilon)^{|v u|}
        |v r u| \epsilon.$
        % $$
        % ||\hat \rho(v r u) - I||_F < (\sqrt{n} + \epsilon)^{|v r
        % u|} |v r u| \epsilon.
        % $$

        Dokaz izvedemo z indukcijo po dolžini besede $v r u$.
        Za $|v r u| = |r|$ je po predpostavki
        $||\hat \rho(r) - I||_F < \epsilon < |r|\epsilon$. %
        % (\sqrt{n}+ \epsilon)^{|r|}
        ...
        \TODO{a rabimo upoštevat tud generatorje oblike
        $u^{-1} r ^{-1} v^{-1}$? Ne, ker so besede samo nad $S$?}
      ...
      \begin{lema}
        ...
        < \frac{\left( 1 + (\sqrt{n} + \epsilon)^{|w|} |w| \epsilon
      ...
      \begin{lema}
        ...
        $\Loss{rel}(\hat \rho)  < \text{TODO}$.
        Potem za vsak $g, h \in G$ velja
        $$
        ||\hat \rho(gh) - \hat \rho(g) \hat \rho(h)||_F < \text{TODO}.
        $$
      \end{lema}
      \begin{dokaz}
        ...
        Ocenimo
        \begin{align*}
          ||\hat \rho(w_g w_h) -   \hat \rho(w_{gh})||_F
          &\leq
          ||\hat \rho(w_{gh}^{-1}) ||_F||\hat \rho(w_g w_h) \hat
          \rho(w_{gh})^{-1} - I||_F
          %
          %&=
          %||\hat \rho(w_g w_h) -   \hat \rho(w_{gh})||_F \\
          ...
          &\leq
          ||\hat \rho(w_{gh}^{-1}) ||_F||\hat \rho(w_g w_h) \hat
          \rho(w_{gh})^{-1}-   \hat \rho(w_g w_h) \hat \rho(w_{gh}^{-1})||_F
          \\
          & +
          ||\hat \rho(w_{gh}^{-1}) ||_F||\hat \rho(w_g w_h) \hat
          \rho(w_{gh}^{-1}) - I||_F \\
          ...
          ||\hat \rho(w_{gh}^{-1}) ||_F
          ||\hat \rho(w_g w_h)||_F
          ||\hat \rho(w_{gh})^{-1} - \hat \rho(w_{gh}^{-1})||_F \\
          &+
          ||\hat \rho(w_{gh}^{-1}) ||_F
          ||\hat \rho(w_g w_h) \hat \rho(w_{gh}^{-1}) - I||_F \\
          &= o(\epsilon).
          %
        \end{align*}
      \end{dokaz}
      ...
      \begin{dokaz}
        ...
        \begin{align*}
          ...
          + ||\hat \rho(gh) - \hat \rho(g)\hat \rho(h)||_F \\
          &+
          ||\hat \rho(g)\hat \rho(h) - \mu(g)\mu(h)||_F \\
          &=
          o(\epsilon).
        \end{align*}
      \end{dokaz}
      ...
      \begin{lema}
        ...
        velja
        $$
        ||\hat \rho(s) - \rho(s)||_F < o(\epsilon)
        $$
        in
        $$
        ||\hat \rho(s) -\mu(s)||_F < o(\epsilon).
        $$
      \end{lema}
      ...
      \ref{images/dihedral/originals/cropped/Dn_1dim_n1.png}.
      %%%%%%%%%%%%%
      \begin{figure}[ht]
        \centering
      ...
      \begin{align*}
        \Loss{rel} &= \frac{1}{3} \left( ||S^2 -1||_F + ||R^n
        -1||_F  + ||(RS)^2 -1||_F  \right )\\
        \Loss{unit} &=  \frac{1}{2} \left ( ||R^2 -1||_F + ||S ^2
        -1||_F  \right)  \\
        \Loss{irr} &= \left ( \frac{1}{2n}\sum_{i=0}^n(||R^i||_F +
        ||R^iS||_F)    - 1\right )^2.
      \end{align*}
      \paragraph{Numerične rešitve}
      ...
      $(\operatorname{tr}(R(t), \operatorname{tr(S(t)})$.
          ...
          \TODO{Kaj so meje v splošnem?}
          %Označimo s
          %$\phi_{\phi_0}(t)$ rešitev enačbe gradientnega toka z začetnimi
          %parametri $\phi_0$. Definiramo
          %$V(x)   = \frac{d}{dt}|_{t=0} \phi_{\phi_0}(t)$.
          %\TODO{Verjetno so meje ravno $x$, kjer $V$ nima polnega ranga??}
          ...
          oblike $z_G^{\operatorname{red}(z_G)}$ in simetrično za
          $z_G$. Označimo
          ...
          \TODO{znotraj kvadrantov maš poleg štirih rešitev $\pm 1$
            lahko še kakšne rešitve - znam zkonstruirat, daj primer.
            Ampak ostale limite so sam pike.
          }
          ...
          \hat\rho(s)$ definira slučajno preslikavo\footnote{Izraba
            ...
            $$P(s = f) = \prod _{i=1}^n P(s(i) = f(i)$$
              ...
              \begin{lema}
                ...
                $\Loss{rel}(\hat\rho) < \delta$ sledi $P_r = I +
              ...
              \begin{dokaz}
                ...
                \begin{align*}
                  - \frac{1}{|R|} \sum_{r \in
                  R} \operatorname{tr}(\log(P_r)) &< \delta \\
                  -\log(\prod_{i=1}^n(P_r)_{i,i}) &< \delta |R| \\
                  \log(P(r = \operatorname{id})) &> -\delta |R| \\
                  1 - P(r = \operatorname{id}) &< 1 - e^{-\delta |R|} =: \gamma.
                \end{align*}

                Ocenimo
                $$ 1 - (P_r)_{i,i} \leq  1 - (P_r)_{1,1}
                (P_r)_{2,2} \dotsm (P_r)_{n,n}
                = 1 - P(r=\operatorname{id})< \gamma.$$
                %
                $P_r$ je stohastična, torej velja
                $\sum_{j=1}^n (P_r)_{i,j} = 1$. Iz tega sledi
                $\sum_{j \neq i}
                (P_r)_{i,j} < \gamma$. Sledi
                $$
                P_{i, j} < \sum_{j \neq i} P_{i,j} < \gamma,
                $$
                za $i \neq j$. Iz tega sledi $||E_r||_F < n
                \gamma = n(1 - e^{-\delta |R|})$. Za dovolj majhno
                izbiro $\delta > 0$ je $||E_r||_F < \epsilon$.
              \end{dokaz}
              \begin{lema}[Za dovolj majhen $\epsilon$ je $P_s$ obrlnjiva]
                \label{lema : Ps je obrnljiva}
                Obstaja $\epsilon_0$, da za vsak $0 < \epsilon <
                \epsilon_0$ in $P_r = I + E_r$ z $||E_r||_F <
                \epsilon$ velja, da je $P_s$ obrnjiva za vsak
                generator $s \in S$.
              \end{lema}
              \begin{dokaz}
                Vemo, da obstaja $\varepsilon_0$, da je za
                $||E||_F < \epsilon$ matrika $I + E$ obrnljiva.
                (Vzameš zvezno preslikavo
              ...
              \begin{dokaz}
                ...
                Na množici $Z$
                je torej $g$ neničelen, iz česar sledi $\min_{(X,Y)
                \in Z}g(X,Y) = c > 0$.
              ...
              \begin{dokaz}
                ...
                \overline{P_{s_m}} = I.
                $$
                Če je izguba nad relacijami dovolj majhna,
                preko leme \ref{lema:Pr blizu I}
                sledi, da je
                \begin{equation}
                  \label{eq : P_r ~ id}
                ...
                %
                %$$
                za $||E_r||_F < \epsilon$. Dodatno lema  \ref{lema :
                Ps je obrnljiva}
                zagotovi, da za dovolj majhno vrednost $\Loss{rel}(\hat\rho)$
                ...
                ||P_{s_1}^{-1}||\epsilon$. Če je $\epsilon$ dovolj
                majhen, je po lemi \ref{lema:inv~stohasticen->inv~permutacijska}
                $$
                ...
                P_{s_1}^{-1}E_r P_{s_1},
                $$
                iz česar za $||E_{s_2}|| < \epsilon_1$ sledi
                $P_{s_2} = \overline{P_{s_2}} + E_{s_2}$.
                Induktivno nadaljujemo. Za vsak $j \in [m]$ obstaja
                $||E_{s_j}|| < \epsilon_j$, da je
                $$
                ...
                ne morejo biti manjše od $1$. Iz tega sledi, da je
            ...
            \begin{definicija}
              ...
              iskanje minimuma
              $$
              \operatorname{argmin}_{P \in \Pi} ||M_1 - PM_2P^T ||_F,
              $$
            ...
            P^T||_F$ nad večje družine matrik $P$ in prevedbo
            ...
            %
            \begin{lema}
              Obstaja $\epsilon > 0$, da iz
              $\Loss{unitary}(P) < \epsilon$
              sledi, da je  $\overline{P}$
            ...
            \end{dokaz}
            \begin{lema}
              Obstaja $\epsilon_0$, da iz
              $$
              \loss_\sim(P) < \epsilon_0
              $$
            \end{lema}
            Opazujmo funkcijo izgube
            $f \colon X \mapsto || M_1 - X M_2 X^T ||_F$.
            ...
            $$(p_{i, 1}, \dotsc, x_{i, n - i + 1}) =
            \operatorname{softmax}(x_{i, 1}, \dotsc, x_{i, n - i + 1})$$,
            kjer je $p_{i,j}$ verjetnost, da $i$ postavimo na $j$-to prosto mesto, če smo prej 
            postavili $i-1$ elementov.
            Velja 
            $$
              M_{\text{blaga preparametrizacija}} =
              \operatorname{Softmax}_\text{po blokih} \circ W_2 \circ \operatorname{ReLU} \circ W_1,
            $$
            ...
            $$
            \operatorname{flatten}(P_d) = M_{\text{blaga preparametrizacija}}(x_0).
            $$
            Vektor $x_0 \in \R^n$ in afini preslikavi $W_1, W_2$ so parametri modela.

            \emph{Globoko preparametrizacijo} definiramo definiramo kot $5$ plastno nevronsko mrežo 
            z vohdno in skritimi plastmi velikosti $n^2$ in izhodno plastjo velikosti
            $\frac{n^2 - n - 2}{2}$. Aktivacijske funkcije v skritih plasteh so $\operatorname{ReLu}$, na zadnjih plasteh 
            ...
            (y_0).
            $$,
            za parametre modela vzamemo vektor $y_0 \in \R^{n^2}$ in afine preslikave
            $W_i$.
            ...
            \begin{figure}
              \centering
              \includegraphics[width=0.7\linewidth]{images/graphs/comparisons/average_sample45_thesis.pdf.pdf}
              \caption{
                \TODO{EXTENDED VERSION!}
                Primerjava napak različnih metod glede
              na velikost grafa.}
              \label{fig:loss_computation_time_comparison_nn}
            \end{figure}
            ...
            \appendix
            \clearpage
            \section{Konvergenca globokih mrež}
            ...
            poljubna $\sigma$-\emph{gradient dominantna} funkcija
            izgube $\loss (\phi) $ postaja vse
