...
def plot_cyclic(Ps_initial, Ps_final, loss, title, filename, remove_axes=True):
    ...
    gs = fig.add_gridspec(nrows=2, ncols=3, width_ratios=[1, 1, 0.05], height_ratios=[0.01, 1])
    ax0 = fig.add_subplot(gs[1, 0])
    ax1 = fig.add_subplot(gs[1, 1])
    cax = fig.add_subplot(gs[1, 2])

    im0 = ax0.imshow(Ps_initial, cmap=blueorange, vmin=vmin, vmax=vmax)
    ax0.set_title("Začetni parametri $P_s$",)
    im1 = ax1.imshow(Ps_final, cmap=blueorange, vmin=vmin, vmax=vmax)
    ax1.set_title(f"Končni parametri $P_s$")
...
def plot_dihedral(Pr_initial, Pr_final, Ps_initial, Ps_final, loss, title, filename, remove_axes=True):
    ...
    fig.suptitle(title + f"; končna napaka: {loss}", fontsize=14)
